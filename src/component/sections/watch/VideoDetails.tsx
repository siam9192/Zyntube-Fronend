...
`
 ...
 description =  description.replace(/(https?:\/\/[^\s]+)/g,(url)=>`<a class="description__url" href="${url}" target="_blank">${url}</a>`)

  return (
    <div className='py-5'>
        <h1 className='md:text-2xl text-xl font-medium text-black'>Jhol | Coke Studio Pakistan | Season 15 | Maanu x Annural Khalid</h1>
        <div className='mt-3'>
     <div className='flex  md:flex-row flex-col justify-between md:items-center md:gap-0 gap-4'>
     <div className='flex items-center gap-4 '>
                <Avatar url='https://yt3.ggpht.com/1MuvHPUzufSwNRsvbbIiuPD9A7gMvJwvoCJX2xmql2PficU-vsSdprlOXpuzrdIpQyMJSRUEjA=s48-c-k-c0x00ffffff-no-rj' className='size-12 rounded-full'/>
                <div>
                    <p className='md:text-xl text-lg text-black font-medium'>Coke Studio Pakistan <span ><BsPatchCheckFill className='inline' size={18} color=''/></span></p>
     ...
     <div onClick={()=>setIsViewFull(!isViewFull)} className='mt-7 p-5 bg-gray-100 rounded-md  hover:cursor-default '>
      ...
      <p  dangerouslySetInnerHTML={{__html:description}} className={`text-[0.9rem] text-gray-900 leading-[1.8rem] ${!isViewFull ? ' line-clamp-5':''} `}/>
