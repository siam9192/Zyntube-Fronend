    ...
    <div className="py-3 px-2 border-r h-full border-r-gray-700/10">
      <div>
        <Avatar
          ...
          className=" lg:size-20 xl:size-32 rounded-full mx-auto"
        />
      ...
      <div className="space-y-2 w-full p-3 border-t border-gray-700/10 mt-4">
        {routes.map(route => (
          <Link to={route.href} key={route.title} className="block w-full">
            <button
              className={`flex lg:justify-center xl:justify-start  items-center gap-3 lg:p-4 xl:rounded-md rounded-full xl:px-2 xl:py-3 hover:bg-gray-50 ${pathname === route.href ? 'bg-gray-100 ' : ''} xl:hover:rounded-md lg:mx-auto xl:w-full `}
            >
