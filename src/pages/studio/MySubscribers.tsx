...
const MySubscribers = () => {
  ...
  const tabs = ['Latest', 'Oldest', 'Name A-Z', 'Name Z-A'];
  return (
    <div>
      <h1 className="md:text-3xl text-2xl text-black font-semibold font-primary">Subscribers</h1>
      <div className="mt-5 flex lg:flex row flex-col lg:items-center justify-between lg:gap-0 gap-2">
        ...
        <div>
          <p className="text-xl  lg:text-start text-end">
            <span className="text-primary">{(763738).toLocaleString()}</span> Subscribers
          </p>
        </div>
      </div>
      <div>
      ...
      <div className="mt-10 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 md:gap-5">
        {Array.from({ length: 30 }).map((sub, indx) => (
          <div
            key={indx}
            className="flex  flex-col md:flex-row  gap-2 md:p-5 p-2 border-2 rounded-md border-gray-600/10"
          >
            <img
            ...
            />
            <div>
              <p className="font-medium text-lg ">Excited boy gaming</p>
              <p className="font-medium  text-gray-600">120K</p>
              <p className="text-sm text-gray-700 ">Since 10,2,2014</p>
            </div>
          </div>
        ))}
      </div>
